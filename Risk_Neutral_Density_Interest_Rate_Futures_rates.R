...
call <- function(x, KC){      #call price on the contract in the B&S model as a put on the euribor rate
  d1_C <- (x[1] + x[2]^2 - log(100 - KC))/x[2]
  d2_C <- d1_C - x[2]
  call <- exp(-r*T)*(- exp(x[1] + (x[2]^2/2))*pnorm(-d1_C) + (100 - KC)*pnorm(-d2_C) )}

esp <- function(x){100 - exp(x[1] + (x[2]^2/2))}          #expected value of the euribor rate
...
for (m in 1:length(charac$option_matu)){
  ...
  start <- rep(c(log(100 - FWD), 0.2), each = nb_log)
  lower <- rep(c(0, 1e-6), each = nb_log)
  upper <- rep(c(2, 0.9), each = nb_log)
  ...
  if(length(PARA) > 0 ){ 
    ...
    params[[m]] <- c(log(100 - FWD) + (solu$par[1:2] - log(100 - FWD))/T, solu$par[3:4]/sqrt(T), solu$par[5])
  ...
  range_px[[m]] <- range(100 - KC)                                        #the range of strike for matu m
...
E_y <- mapply(function(x, y) sum(rollmean(x*y, 2)*diff(x)), PX_2, DNR_2)

moments <- function(x){
  return(mapply(function(x, y, z, t) sum(rollmean( ( (t - y)^x )*z, 2)*diff(t)), x, E_y, DNR_2, PX_2))}
...
for (i in 1:length(params_2)){
  ...
  for (j in 1:length(thres)){
    quantiles[[i]][[j]] <- 1e-2*mean(PX_2[[i]][c(min(which(NCDF[[i]] > thres[j] - 1e-5)),
...
ggplot() + geom_line(data = df_p, aes(x = price, y = density, group = maturity, color = maturity)) +
  labs(title = "3-month Euribor prices (%)", subtitle = "Implied Probability Density Functions (PDF) at all options' maturities") +
  labs(y = "PDF", x = "Euribor future rate (%)") + 
...
ggplot() + geom_line(data = df_p, aes(x = price, y = density, group = maturity, color = maturity)) +  #group/fill
  geom_bar(stat = "identity") + labs(x = "future rates (%)", y = "PDF") + facet_wrap(~maturity) + theme_bw() +
  theme(legend.position = "none", plot.margin = margin(.8,.5,.8,.5, "cm")) +
  labs(title = "3-month Euribor future rates (%)", subtitle = "Implied Probability Density Functions (PDF) at all options' maturities")
...
#graph of quantile of order q for d^th maturity other method
quantiles_t_6 <- quantiles_2 %>% filter(row_number() == 6)
temporary_plot <- quantiles_t_6 %>% t %>%  tibble %>% rename_with(~"x") %>%  ggplot() + geom_density(aes(x = x))

build <- ggplot2::ggplot_build(temporary_plot)

df_breaks <- build$data[[1]] %>% 
  mutate(status = case_when(x <= quantiles_t_6 %>% select(q50) ~ 'q_5%', TRUE ~ ''))

stress <- tibble( quantiles_t_6 %>% select(q50)) %>% as.numeric

df_breaks %>% ggplot() + geom_area( aes(x = x, y = y, fill = status), show.legend = FALSE) +
  annotate(geom = 'text', x = stress, y = -0.25, hjust = 0.5, label = paste0("q5: ", 100*round(stress, 4),"%")) + 
  labs(title = "Euribor 3 month RND and quantile of order 90 %",
       subtitle = "proba density 2024-06-17", x = "Euribor 3-month future price (%)", y = "proba density") +
  scale_x_continuous(labels = scales::percent) + theme(plot.margin = margin(1.2,.5,1.2,.5, "cm")) + theme_light()